// MelodyHub interior — text-reveal（已换肤）
// 来源：ddoemonn/interior components/interior/text-reveal.tsx
// 行为（useTextReveal 逐词/逐字拆分 + stagger/maxDuration + useInView/once/amount + reduced-motion）原样保留，
// 仅做：删 "use client"、删 Tailwind className 改走 text-reveal.css（CSS Vars）。
...
export function TextReveal({
  ...
  return (
    <span ref={ref} className={`mh-text-reveal ${className}`}>
      <span className="mh-text-reveal__sr">{text}</span>

      <span aria-hidden="true">
        {groups.map((group, g) => (
          <Fragment key={group.key}>
            {g > 0 ? ' ' : null}
            <span className="mh-text-reveal__word">
              {group.units.map((unit) => (
                <motion.span
                  key={unit.key}
                  className="mh-text-reveal__unit"
