// MelodyHub interior — tooltip-group（已换肤）
// 来源：ddoemonn/interior components/interior/tooltip-group.tsx
// 上游 commit 见 _vendor/SOURCE.txt；行为（TooltipStore 延迟/暖机/行程、键盘可达、reduced-motion）原样保留，
// 仅做：删 "use client"、删 Tailwind className 改走 tooltip-group.css（CSS Vars）。
import {
  cloneElement,
  createContext,
  useContext,
...
export function TooltipGroup({
  ...
  return (
    <TooltipGroupContext.Provider value={store}>
      {className ? <div className={`mh-tooltip-group ${className}`}>{children}</div> : children}
...
export function Tooltip({
  ...
  return (
    <span className={`mh-tooltip${className ? ` ${className}` : ''}`}>
      {trigger}

      <span
        aria-hidden={!open}
        data-side={side}
        className={`mh-tooltip__anchor mh-tooltip__anchor--${side}`}
      >
        <AnimatePresence>
          {open && (
            <motion.span
              ...
              className={`mh-tooltip__content${contentClassName ? ` ${contentClassName}` : ''}`}
            >
              <motion.span
                aria-hidden
                layout={!reduced}
                transition={reduced ? { duration: 0 } : GLIDE}
                className="mh-tooltip__surface"
              />
              <motion.span
                layout={reduced ? false : 'position'}
                initial={
                  reduced ? false : skipped ? { opacity: 0, x: travel * 14, y: 0 } : { opacity: 0, x: 0, y: 9 }
                }
                animate={{ opacity: 1, x: 0, y: 0 }}
                transition={reduced ? { duration: 0 } : SWAP}
                className="mh-tooltip__label"
              >
                {label}
              </motion.span>
            </motion.span>
