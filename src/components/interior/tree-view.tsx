// MelodyHub interior — tree-view（已换肤）
// 来源：ddoemonn/interior components/interior/tree-view.tsx
// 上游 commit 见 _vendor/SOURCE.txt；行为（useTreeView 展开/选中受控与非受控、
// roving tabindex、Arrow/Home/End/typeahead 键盘导航、role=tree ARIA、
// AnimatePresence 高度/透明度动画 + reduced-motion）原样保留，
// 仅做：删 "use client"、删 Tailwind className 改走 tree-view.css（CSS Vars）、
// 内联 svg caret 换成 lucide-react ChevronRight（旋转动画保留）。
...
export function TreeView({
  ...
  const renderNodes = (list: TreeNode[], level: number) =>
    list.map((node, i) => {
      ...
      return (
        <li key={node.id} role="none" className="mh-tree-view__node">
          <div
            role="treeitem"
            ...
            className="mh-tree-view__row"
          >
            {row.branch ? (
              <Caret open={row.open} />
            ) : (
              <span className="mh-tree-view__spacer" />
            )}

            <span
              data-selected={isSelected}
              className="mh-tree-view__label"
            >
              {node.label}
            </span>

            {node.meta ? (
              <span className="mh-tree-view__meta">{node.meta}</span>
            ) : null}
          </div>
            ...
            <AnimatePresence initial={false}>
              {row.open ? (
                <motion.ul
                  ...
                  className="mh-tree-view__group"
                >
                  <div className="mh-tree-view__indent">
  ...
  return (
    <div className={`mh-tree-view ${className}`}>
      <ul role="tree" aria-label={label} className="mh-tree-view__list">
        {renderNodes(nodes, 1)}
      </ul>
      <span id={hintId} className="mh-tree-view__sr">
