// MelodyHub interior — command-palette（已换肤）
// 来源：ddoemonn/interior components/interior/command-palette.tsx
// 上游 commit 见 _vendor/SOURCE.txt；useCommandPalette hook 原样保留（含模糊评分过滤、
// 键盘导航、pinned 高亮、指针去抖激活、滚动跟随、空状态 live-region、防抖计数），
// PaletteLayer 原样保留（含 portal、Escape 捕获、滚动锁、backdrop 点按判定、进出场动效、reduced-motion）。
// 仅做：删 "use client"、删 Tailwind 改走 command-palette.css（CSS Vars）、
// 搜索放大镜内联 SVG 改 lucide Search。
...
export function CommandPalette({
  ...
  const surface = (
    <div
      ref={panelRef}
      className={`mh-cmdk${overlaid ? ' mh-cmdk--overlay' : ''}${className ? ` ${className}` : ''}`}
    >
      <div className="mh-cmdk__search">
        <Search size={14} className="mh-cmdk__search-icon" aria-hidden />
        <input
          ref={inputRef}
          type="text"
          role="combobox"
          ...
          onKeyDown={onKeyDown}
          className="mh-cmdk__input"
        />
        <span className="mh-cmdk__count">{count}</span>
      </div>
      <div className="mh-cmdk__body" style={{ height }}>
        <ul
          ref={listRef}
          id={`${uid}-list`}
          role="listbox"
            ...
            return (
              ...
              >
                <motion.span
                  ...
                  className="mh-cmdk__row-highlight"
                />
                <span className="mh-cmdk__row-content">
                  <span className="mh-cmdk__label">{item.label}</span>

                  {item.hint ? (
                    <span className="mh-cmdk__hint">{item.hint}</span>
                  ) : null}

                  {item.shortcut ? (
                    <span className="mh-cmdk__shortcuts">
                      {item.shortcut.map((key) => (
                        <span key={key} className="mh-cmdk__kbd">
                          {key}
                        </span>
                      ))}
                    </span>
        ...
        {count === 0 ? (
          <motion.p
            ...
            animate={{ opacity: 1 }}
            transition={reduced ? { duration: 0 } : CROSSFADE}
            className="mh-cmdk__empty"
          >
            {emptyLabel}
          </motion.p>
        ) : null}
      </div>
      <span ref={liveRef} role="status" aria-live="polite" className="mh-cmdk__sr" />
    </div>
  );
    ...
    <AnimatePresence>
      {open ? (
        ...
        >
          <motion.div
            aria-hidden
            className="mh-cmdk-layer__backdrop"
            variants={{
              closed: { opacity: 0 },
              open: {
                opacity: 1,
          ...
          />
          <motion.div
            className="mh-cmdk-layer__panel"
            variants={{
