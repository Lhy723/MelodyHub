// MelodyHub interior — popover（已换肤）
// 来源：ddoemonn/interior components/interior/popover.tsx
// 上游 commit 见 _vendor/SOURCE.txt；行为（usePopover 定位/翻转/钳制 + 失焦/Escape/外部 pointerdown 关闭 + reduced-motion）原样保留，
// 仅做：删除首行客户端指令、删 Tailwind className 改走 popover.css（CSS Vars）。
import {
  useCallback,
  useEffect,
  useId,
...
const ARROW_MOD: Record<PopoverSide, string> = {
  bottom: "mh-popover__arrow--bottom",
  top: "mh-popover__arrow--top",
  right: "mh-popover__arrow--right",
  left: "mh-popover__arrow--left",
};
  ...
  return (
    <>
      <button
        ...
        className={`mh-popover__trigger ${triggerClassName}`}
      >
        {trigger}
      </button>
      <AnimatePresence>
        {open ? (
          <div
            key="popover"
            ref={floatingRef}
            className="mh-popover__floating"
            onBlurCapture={(event) => {
          ...
          >
            <motion.div
              ...
              className={`mh-popover__panel ${className}`}
            >
              <span
                ref={arrowRef}
                aria-hidden
                style={{ width: arrowSize, height: arrowSize, transform: "rotate(45deg)" }}
                className={`mh-popover__arrow ${ARROW_MOD[at]}`}
              />
              <div ref={contentRef} className="mh-popover__content">
                {children}
              </div>
            </motion.div>
          </div>
