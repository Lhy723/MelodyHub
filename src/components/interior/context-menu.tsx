// MelodyHub interior — context-menu（已换肤）
// 来源：ddoemonn/interior components/interior/context-menu.tsx
// 上游 commit 见 _vendor/SOURCE.txt；useContextMenu hook 原样保留（含右键定位/视口钳制、
// ArrowUp/Down/Home/End/Tab/Escape 键盘导航、typeahead、触屏长按打开、失焦/滚动/resize/blur
// 关闭、焦点恢复与 reduced-motion），仅做：删 "use client"、删 Tailwind 改走
// context-menu.css（CSS Vars）、motion/react 不变。
// 说明：上游本文件内无 phosphor/内联 svg 图标（图标由调用方经 item.icon: ReactNode 传入，
// 原样透传），故无图标替换；上游为扁平 items + separator，无子菜单结构，原样保留。
import {
  useCallback,
  useEffect,
  useId,
...
function measure(items: ContextMenuItem[]): number {
  ...
  return (
    <>
      <div
        ...
        className={`mh-context-menu__trigger ${className}`}
      >
        {children}
        <span id={`${uid}-hint`} className="mh-context-menu__hint">
        ...
        <AnimatePresence>
          {placement ? (
            <motion.div
              ...
              className="mh-context-menu__menu"
            >
              {items.map((item, index) =>
                item.type === 'separator' ? (
                  <div key={item.id} className="mh-context-menu__separator">
                    <hr className="mh-context-menu__separator-line" />
                  </div>
                ) : (
                  <button
                    key={item.id}
                    ...
                    {...getItemProps(index)}
                    className={`mh-context-menu__item${active === index ? ' is-active' : ''}${item.disabled ? ' is-disabled' : ''}`}
                  >
                    {hasIcons ? (
                      <span aria-hidden className="mh-context-menu__icon">
                        {item.icon}
                      </span>
                    ) : null}

                    <span className="mh-context-menu__label">{item.label}</span>
                    {item.shortcut ? (
                      <span aria-hidden className="mh-context-menu__shortcut">
                        {item.shortcut}
                      </span>
                    ) : null}
                  </button>
