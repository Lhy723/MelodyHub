// MelodyHub interior — copy-button（已换肤）
// 来源：ddoemonn/interior components/interior/copy-button.tsx
// 上游 commit 见 _vendor/SOURCE.txt；行为（useCopyToClipboard + 图标交叉淡入 + reduced-motion）原样保留，
// 仅做：删 "use client"、删 Tailwind className 改走 copy-button.css（CSS Vars）、文案默认中英由调用方传入。
...
export function CopyButton({
  ...
  return (
    <motion.button
      ...
      transition={CELL}
      className={`mh-copy-btn ${className}`}
    >
      <span className="mh-copy-btn__icon" aria-hidden="true">
        <motion.svg
          viewBox="0 0 14 14"
          fill="none"
          stroke="currentColor"
      ...
      <span aria-hidden="true" className="mh-copy-btn__labels">
        {labels.map(([key, text]) => (
          <motion.span
            key={key}
            initial={false}
            animate={key === status ? { opacity: 1, y: 0, filter: 'blur(0px)' } : { opacity: 0, y: 3, filter: 'blur(3px)' }}
            transition={fade}
          >
            {text}
          </motion.span>
      ...
      <span role="status" aria-live="polite" className="mh-copy-btn__sr">
