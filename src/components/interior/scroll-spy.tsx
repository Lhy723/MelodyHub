// MelodyHub interior — scroll-spy（已换肤）
// 来源：ddoemonn/interior components/interior/scroll-spy.tsx
// 上游 commit 见 _vendor/SOURCE.txt；行为（useScrollSpy 区段测量/滚动追踪/
// 高亮指示器动画/点击滚动锁定/reduced-motion/aria-live 播报）原样保留，
// 仅做：删 "use client"、删 Tailwind className 改走 scroll-spy.css（CSS Vars）。
// 本组件无 phosphor 图标，无需 lucide-react 替换；motion/react 不变。
import { useCallback, useEffect, useId, useRef, useState } from 'react';
import { motion, useReducedMotion } from 'motion/react';
import './scroll-spy.css';
...
export function ScrollSpy({
  ...
  return (
    <nav aria-label={label} className={`mh-scroll-spy ${className}`}>
      <div className="mh-scroll-spy__track">
        <ol className="mh-scroll-spy__list">
              ...
              >
                {active ? (
                  <motion.span
                    ...
                    aria-hidden
                    transition={CELL}
                    className="mh-scroll-spy__thumb"
                  />
                ) : null}
                ...
                <a
                  ...
                  data-active={active}
                  className="mh-scroll-spy__link"
                >
                  <span className="mh-scroll-spy__label-wrap">
                    <span
                      aria-hidden
                      className="mh-scroll-spy__label-ghost"
                    >
                      {section.label}
                    </span>
                    <span className="mh-scroll-spy__label">
                      {section.label}
                    </span>
                  </span>
                </a>
      ...
      <p aria-live="polite" className="mh-scroll-spy__sr">
        {announce}
      </p>
    </nav>
  );
