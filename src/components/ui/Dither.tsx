...
// ═══════════════════════════════════════════════════════════════
// Dither — React Bits background component
// (https://reactbits.dev/backgrounds/dither)
//
// TypeScript port of the official source. Renders an animated
// dithered wave field via a custom shader on a full-screen plane,
// then applies an 8x8 Bayer dithering postprocessing Effect.
// ═══════════════════════════════════════════════════════════════
...
interface RetroEffectImplUniforms {
  colorNum: THREE.Uniform;
  pixelSize: THREE.Uniform;
}

class RetroEffectImpl extends Effect {
  declare uniforms: Map<string, THREE.Uniform>;
  constructor() {
    const uniforms: RetroEffectImplUniforms = {
      colorNum: new THREE.Uniform(4.0),
      pixelSize: new THREE.Uniform(2.0),
    };
    const uniformsMap = new Map<string, THREE.Uniform>([
      ['colorNum', uniforms.colorNum],
      ['pixelSize', uniforms.pixelSize],
    ]);
    super('RetroEffect', ditherFragmentShader, { uniforms: uniformsMap });
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<Record<string, THREE.Uniform>>({
