...
type CodexSettingGroup =
  'model' | 'permissions' | 'tools' | 'features' | 'agents' | 'integrations' | 'tui' | 'environment' | 'advanced';
...
const ROOT_SPECS: CodexSettingSpec[] = [
  ...
  { key: 'model_auto_compact_token_limit', kind: 'integer', group: 'model' },
  {
    ...
    kind: 'enum',
    group: 'model',
    options: ['total', 'body_after_prefix'],
  },
  ...
  { key: 'approval_policy', kind: 'enumOrJson', group: 'permissions', options: ['untrusted', 'on-request', 'never'] },
  {
    ...
    kind: 'enum',
    group: 'permissions',
    options: ['user', 'auto_review', 'guardian_subagent'],
  },
  ...
  // ── Codex 0.154 的子表键 ──
  { key: 'browser_use.allow_history_access', kind: 'boolean', group: 'integrations' },
  { key: 'browser_use.default_origin_policy.access', kind: 'string', group: 'integrations' },
  { key: 'browser_use.default_origin_policy.downloads', kind: 'string', group: 'integrations' },
  { key: 'browser_use.default_origin_policy.uploads', kind: 'string', group: 'integrations' },
  { key: 'browser_use.default_origin_policy.full_cdp_access', kind: 'string', group: 'integrations' },
  { key: 'computer_use.default_app_access', kind: 'string', group: 'integrations' },
  { key: 'computer_use.macos.bundle_ids', kind: 'string', group: 'integrations' },
  { key: 'computer_use.windows.aumids', kind: 'string', group: 'integrations' },
  { key: 'goals.max_goal_token_budget', kind: 'string', group: 'integrations' },
  { key: 'allow_symlinked_codex_home', kind: 'boolean', group: 'permissions' },
  { key: 'browser_use', kind: 'json', group: 'integrations' },
  { key: 'computer_use', kind: 'json', group: 'integrations' },
  { key: 'mcp_optional_startup_grace_ms', kind: 'integer', group: 'integrations' },
  { key: 'thread_unload_delay_secs', kind: 'integer', group: 'advanced' },
  { key: 'js_repl_node_path', kind: 'string', group: 'advanced' },
  { key: 'js_repl_node_module_dirs', kind: 'json', group: 'advanced' },
  { key: 'profile', kind: 'string', group: 'advanced' },
  { key: 'profiles', kind: 'json', group: 'advanced' },
  { key: 'responses_api_metadata', kind: 'json', group: 'advanced' },
  { key: 'experimental_thread_store_endpoint', kind: 'string', group: 'advanced' },
  { key: 'goals', kind: 'json', group: 'agents' },
  ...
  { key: 'agents.job_max_runtime_seconds', kind: 'integer', group: 'agents' },
  { key: 'agents.roles', kind: 'json', group: 'agents' },
  { key: 'agents.max_depth', kind: 'integer', group: 'agents' },
  { key: 'features.tool_registry', kind: 'json', group: 'features' },
  { key: 'features.guardianv2', kind: 'json', group: 'features' },
  { key: 'features.context_management', kind: 'json', group: 'features' },
  { key: 'features.sleep_tool', kind: 'json', group: 'features' },
...
const FEATURE_EXTRA_SPECS: CodexSettingSpec[] = [
  ...
  {
    key: 'features.current_time_reminder.clock_source',
    kind: 'enum',
    group: 'features',
    options: ['system', 'external'],
  },
  {
    key: 'features.current_time_reminder.delivery_mode',
    kind: 'enum',
    group: 'features',
    options: ['any_inference', 'after_user_or_tool_output'],
  },
...
const COMPLEX_ROOT_SPECS: CodexSettingSpec[] = [
  ...
  { key: 'skills.max_context_tokens', kind: 'integer', group: 'integrations' },
  ...
  { key: 'tui.whimsy', kind: 'boolean', group: 'tui' },
  { key: 'tui.auto_recap', kind: 'boolean', group: 'tui' },
  { key: 'tui.question_esc_back', kind: 'boolean', group: 'tui' },
...
const CODEX_LABELS: Record<string, { label: string; hint?: string }> = {
  ...
  allow_symlinked_codex_home: {
    label: '允许 CODEX_HOME 软链接',
    hint: '允许 CODEX_HOME 指向软链接目录（同步盘/自定义路径场景）。',
  },
  mcp_optional_startup_grace_ms: {
    label: '可选 MCP 启动宽限 (ms)',
    hint: '可选 MCP 服务的启动等待上限，超时不再阻塞启动。',
  },
  thread_unload_delay_secs: { label: '会话卸载延迟 (秒)', hint: '空闲会话多久后卸载。' },
  js_repl_node_path: { label: 'JS REPL Node 路径', hint: 'js_repl 使用的 node 可执行文件。' },
  js_repl_node_module_dirs: { label: 'JS REPL 模块目录 (JSON)', hint: 'js_repl 查找模块的目录列表。' },
  profile: { label: '当前配置档案', hint: '启用 profiles 中的哪一个档案。' },
  profiles: { label: '配置档案 (JSON)', hint: '可切换的配置档案集合，结构同顶层配置。' },
  responses_api_metadata: { label: 'Responses 请求元数据 (JSON)', hint: '随请求发送的键值元数据。' },
  experimental_thread_store_endpoint: { label: '实验性会话存储端点', hint: '实验性远端会话存储服务地址。' },
  'agents.job_max_runtime_seconds': { label: '子代理最长运行 (秒)', hint: '子代理任务的最长运行时间。' },
  'agents.roles': { label: '代理角色 (JSON)', hint: '自定义代理角色定义。' },
  'tui.whimsy': { label: '趣味文案', hint: 'TUI 的趣味提示与动画。' },
  'tui.auto_recap': { label: '自动回顾', hint: '自动生成会话要点回顾。' },
  'tui.question_esc_back': { label: 'Esc 返回上一问', hint: '回答提问时按 Esc 回退。' },
  'features.tool_registry': { label: '工具注册表 (JSON)', hint: '工具注册与冲突处理配置。' },
  'features.guardianv2': { label: 'Guardian v2 (JSON)', hint: '新版 Guardian 审查配置。' },
  'features.context_management': { label: '上下文管理 (JSON)', hint: '上下文提醒与裁剪策略。' },
  'features.sleep_tool': { label: '休眠工具 (JSON)', hint: 'sleep 工具相关配置。' },
  'skills.max_context_tokens': { label: '技能上下文上限', hint: '注入技能说明占用的最大 token 数。' },
  'browser_use.allow_history_access': { label: '是否允许浏览器工具读取历史记录' },
  'browser_use.default_origin_policy.access': { label: '未设置' },
  'browser_use.default_origin_policy.downloads': { label: '未设置' },
  'browser_use.default_origin_policy.uploads': { label: '未设置' },
  'browser_use.default_origin_policy.full_cdp_access': { label: '未设置' },
  'computer_use.default_app_access': { label: '未设置' },
  'computer_use.macos.bundle_ids': { label: '未设置' },
  'computer_use.windows.aumids': { label: '未设置' },
  'goals.max_goal_token_budget': { label: '单个 goal 的最大 token 预算，同时作为新 goal 的默认预算' },
  ...
  'features.current_time_reminder.delivery_mode': {
    label: '提醒交付模式',
    hint: 'any_inference / after_user_or_tool_output',
  },
...
const FEATURE_LABELS: Record<string, { label: string; hint?: string }> = {
  apply_patch_freeform: {
    label: '自由格式 Apply Patch',
    hint: '允许 Codex 以自由文本格式生成补丁，不严格遵循 unified diff 语法。适合处理复杂多文件改动；关闭后必须使用标准 diff 格式。',
  },
  apply_patch_streaming_events: {
    label: 'Apply Patch 流式事件',
    hint: '在应用补丁时以流式方式逐步输出事件，便于 UI 实时显示修改进度。关闭后补丁一次性应用完成才返回结果。',
  },
  apps: {
    label: 'Apps',
    hint: '启用内置 Apps 生态，允许 Codex 调用预装的小应用（如文件管理、终端等）。关闭后仅保留核心能力。',
  },
  auth_elicitation: {
    label: '认证交互',
    hint: '当 MCP 服务器或工具需要认证时，通过交互式弹窗向用户索取凭据。关闭后认证失败将直接报错。',
  },
  browser_use: {
    label: '浏览器操作',
    hint: '允许 Codex 控制内置浏览器执行网页自动化（点击、输入、截图等）。适合需要与网页交互的任务；关闭后无法操作网页。',
  },
  browser_use_external: {
    label: '外部浏览器操作',
    hint: '允许 Codex 通过 CDP 协议控制用户已打开的外部 Chrome/Edge 浏览器，复用已登录的会话。适合需要操作已登录站点的场景。',
  },
  browser_use_full_cdp_access: {
    label: '浏览器完整 CDP 访问',
    hint: '授予 Codex 完整的 Chrome DevTools Protocol 权限，可执行任意浏览器调试命令。安全风险较高，仅在可信环境下启用。',
  },
  chronicle: {
    label: 'Chronicle',
    hint: '启用 Chronicle 子系统记录 Agent 决策时间线，便于事后审计与回放。会增加少量磁盘 IO 开销。',
  },
  code_mode_buffered_exec: {
    label: '代码模式缓冲执行',
    hint: '在代码模式下缓冲命令输出再批量返回，减少频繁刷新。适合长任务；关闭后实时输出每一行。',
  },
  code_mode_only: {
    label: '仅代码模式',
    hint: '强制只允许通过代码执行工具完成任务，禁用自然语言对话。适合纯自动化脚本场景。',
  },
  codex_git_commit: {
    label: 'Codex Git 提交',
    hint: '允许 Codex 自动创建 Git 提交（含生成 commit message）。适合让 Agent 直接完成提交；关闭后仅修改文件不提交。',
  },
  codex_hooks: {
    label: 'Codex Hooks',
    hint: '启用 Codex 专属的 Hook 钩子机制，在工具调用前后执行自定义脚本。与通用 hooks 配合使用。',
  },
  collab: { label: '协作', hint: '启用多人协作模式，多个用户可同时查看和介入同一 Agent 会话。需要网络连接。' },
  collaboration_modes: {
    label: '协作模式',
    hint: '支持多种协作模式（如 review、pair-programming），允许切换 Agent 与人类的协作方式。',
  },
  computer_use: {
    label: '计算机操作',
    hint: '允许 Codex 通过模拟点击、键盘输入等方式操作桌面 GUI 应用。适合需要与图形软件交互的自动化；关闭后仅支持命令行和 API。',
  },
  concurrent_reasoning_summaries: {
    label: '并发推理摘要',
    hint: '在多个推理分支并发执行时为每个分支生成摘要，便于对比。会增加 token 消耗。',
  },
  connectors: {
    label: '连接器',
    hint: '启用 Connector 子系统连接外部数据源（数据库、API 等），让 Agent 可直接查询外部数据。',
  },
  default_mode_request_user_input: {
    label: '默认模式请求用户输入',
    hint: '在默认模式下允许 Agent 主动向用户提问以澄清需求。关闭后 Agent 会基于自身判断继续。',
  },
  deferred_executor: {
    label: '延迟执行器',
    hint: '将命令延迟到批次末尾统一执行，减少沙箱上下文切换开销。适合批量命令场景。',
  },
  deferred_tool_world_state: {
    label: '延迟工具世界状态',
    hint: '延迟同步工具执行后的文件系统状态，提升性能。可能导致 Agent 短暂看到旧状态。',
  },
  elevated_windows_sandbox: {
    label: '提权 Windows 沙箱',
    hint: '在 Windows 上以管理员权限运行沙箱，可执行需要提权的操作（如安装软件）。安全风险较高。',
  },
  enable_experimental_windows_sandbox: {
    label: '启用实验性 Windows 沙箱',
    hint: '启用仍在实验阶段的 Windows 沙箱实现，可能存在稳定性问题。仅在测试时启用。',
  },
  enable_fanout: {
    label: '启用扇出',
    hint: '允许将一个任务拆分成多个子任务并行扇出到不同 Agent，加速大规模任务。会增加 token 消耗。',
  },
  enable_mcp_apps: {
    label: '启用 MCP Apps',
    hint: '允许通过 MCP 协议接入预打包的应用，扩展 Agent 能力（如访问特定 SaaS 服务）。',
  },
  enable_request_compression: {
    label: '启用请求压缩',
    hint: '对发送给上游模型的请求体进行 gzip 压缩，减少网络流量。适合网络带宽受限场景；关闭后使用原始大小。',
  },
  exec_permission_approvals: {
    label: '执行权限审批',
    hint: '在执行高危命令前弹出审批确认框，要求用户确认。建议在共享或生产环境启用。',
  },
  executed_tool_call_metadata: {
    label: '已执行工具调用元数据',
    hint: '记录每个工具调用的元数据（耗时、返回码、参数等），便于审计和调试。会增加少量内存占用。',
  },
  executor_capability_discovery: {
    label: '执行器能力发现',
    hint: '运行时自动探测可用执行器的能力（支持的命令、工具等），让 Agent 选择更合适的执行方式。',
  },
  experimental_use_unified_exec_tool: {
    label: '实验性统一执行工具',
    hint: '使用实验性的统一执行工具替代原有多个独立工具，简化工具调用流程。稳定性未保证。',
  },
  experimental_windows_sandbox: {
    label: '实验性 Windows 沙箱',
    hint: '实验性 Windows 沙箱实现，用于测试新特性。不建议在生产环境使用。',
  },
  external_agent_memory_import: {
    label: '外部 Agent 记忆导入',
    hint: '允许从其他 Agent 导入记忆数据，便于迁移历史上下文。可能存在格式兼容问题。',
  },
  external_migration: {
    label: '外部迁移',
    hint: '启用从外部系统迁移配置和数据的功能，适合从其他工具切换到 Codex 时使用。',
  },
  ...
  guardian_approval: {
    label: '守护者审批',
    hint: '由守护 Agent 在执行前审批敏感操作，提供二次确认。适合安全敏感场景。',
  },
  guardianv2: {
    label: '守护者 v2',
    hint: '守护者审批机制的 v2 版本，改进了审批策略和性能。与 guardian_approval 互斥。',
  },
  hooks: {
    label: 'Hooks',
    hint: '启用通用 Hook 机制，在工具调用、消息收发等事件触发自定义脚本。与 codex_hooks 配合使用。',
  },
  image_detail_original: {
    label: '图片原始细节',
    hint: '在向模型传递图片时保留原始分辨率和细节。会增加 token 消耗；关闭后自动压缩图片。',
  },
  image_generation: {
    label: '图片生成',
    hint: '允许 Codex 调用图片生成模型创建图像。需要上游模型支持；关闭后无法生成图片。',
  },
  imagegenext: {
    label: 'ImageGen 扩展',
    hint: '图片生成能力的扩展版本，支持更多参数（尺寸、风格、批量等）。依赖 image_generation 启用。',
  },
  in_app_browser: {
    label: '应用内浏览器',
    hint: '在 Codex 应用内嵌入浏览器窗口，便于查看网页内容而无需切换外部应用。',
  },
  ...
  local_thread_store_compression: {
    label: '本地线程存储压缩',
    hint: '对本地会话线程存储进行压缩，减少磁盘占用。会增加 CPU 开销。',
  },
  mcp_2026_07_28: {
    label: 'MCP 2026-07-28',
    hint: '启用 2026 年 7 月 28 日发布的 MCP 协议版本特性。可能存在兼容性问题。',
  },
  memories: {
    label: '记忆',
    hint: '启用长期记忆子系统，允许 Agent 跨会话记住用户偏好和历史决策。关闭后每次会话从零开始。',
  },
  memory_tool: {
    label: '记忆工具',
    hint: '为 Agent 提供主动读写记忆的工具，配合 memories 使用。关闭后 Agent 无法主动管理记忆。',
  },
  mentions_v2: { label: '提及 v2', hint: '提及（@）功能的 v2 版本，改进了解析和补全逻辑。与旧版可能存在行为差异。' },
  multi_agent: {
    label: '多 Agent',
    hint: '允许 Codex 启动多个子 Agent 并行处理子任务后汇总结果。适合大型复杂任务；关闭后所有工作由单个 Agent 串行完成。',
  },
  ...
  realtime_conversation: {
    label: '实时对话',
    hint: '启用实时语音/文字对话模式，支持流式双向通信。需要麦克风权限或网络连接。',
  },
  recommended_plugins: { label: '推荐插件', hint: '在插件市场显示基于当前使用习惯的推荐插件。会收集匿名使用数据。' },
  remote_compaction_v2: {
    label: '远程压缩 v2',
    hint: '在服务端进行会话压缩的 v2 版本，减少客户端 token 消耗。需要服务端支持。',
  },
  remote_control: {
    label: '远程控制',
    hint: '允许通过远程连接控制 Codex 实例，便于跨设备协同。存在安全风险，仅在可信网络启用。',
  },
  remote_models: { label: '远程模型', hint: '允许使用远程服务端提供的模型，扩展可用模型列表。需要网络连接。' },
  remote_plugin: { label: '远程插件', hint: '允许从远程服务器加载插件，便于团队共享。需要网络连接。' },
  request_permissions: {
    label: '请求权限',
    hint: '允许 Agent 在需要时主动向用户请求额外权限（如访问特定目录）。关闭后 Agent 仅使用已有权限。',
  },
  request_permissions_tool: {
    label: '请求权限工具',
    hint: '为 Agent 提供请求权限的工具，配合 request_permissions 使用。',
  },
  request_rule: {
    label: '请求规则',
    hint: '启用请求规则引擎，根据预设规则自动审批或拒绝 Agent 的请求。减少人工确认次数。',
  },
  resize_all_images: {
    label: '调整所有图片尺寸',
    hint: '自动调整所有传入图片的尺寸到模型推荐分辨率，减少 token 消耗。关闭后保留原始尺寸。',
  },
  respect_system_proxy: {
    label: '遵循系统代理',
    hint: '自动读取系统代理设置（HTTP_PROXY 等）发送请求。关闭后使用直连。',
  },
  responses_websockets: {
    label: 'Responses WebSocket',
    hint: '通过 WebSocket 传输 Responses API 数据，降低延迟。需要服务端支持。',
  },
  responses_websockets_v2: {
    label: 'Responses WebSocket v2',
    hint: 'WebSocket 传输的 v2 版本，改进了重连和心跳机制。与 v1 互斥。',
  },
  ...
  secret_auth_storage: {
    label: '密钥认证存储',
    hint: '使用系统密钥库（Keychain/Credential Manager）安全存储认证密钥。关闭后使用明文文件存储。',
  },
  shell_snapshot: { label: 'Shell 快照', hint: '在每次 Shell 执行前快照当前环境状态，便于回滚。会增加磁盘占用。' },
  shell_tool: {
    label: 'Shell 工具',
    hint: '允许 Codex 在沙箱中执行本地 Shell 命令（如 ls、git、npm 等）。关闭后 Codex 仅能给出建议，无法实际执行。',
  },
  shell_zsh_fork: {
    label: 'Shell zsh fork',
    hint: '使用 zsh fork 方式执行命令，提升执行隔离性。仅在 zsh 环境下生效。',
  },
  skill_env_var_dependency_prompt: {
    label: 'Skill 环境变量依赖提示',
    hint: '当 Skill 依赖的环境变量缺失时主动提示用户配置。避免 Skill 静默失败。',
  },
  skill_mcp_dependency_install: {
    label: 'Skill MCP 依赖安装',
    hint: '自动安装 Skill 所需的 MCP 服务器依赖。需要网络连接。',
  },
  ...
  standalone_web_search: {
    label: '独立网页搜索',
    hint: '将网页搜索作为独立工具暴露给 Agent，而非内嵌在回答流程中。Agent 可主动决定何时搜索。',
  },
  ...
  terminal_resize_reflow: {
    label: '终端重排',
    hint: '在终端尺寸改变时自动重排历史输出，避免内容错位。适合动态调整窗口大小。',
  },
  terminal_visualization_instructions: {
    label: '终端可视化指令',
    hint: '在终端中渲染可视化指令（图表、进度条等），提升可读性。需要兼容的终端。',
  },
  tool_call_mcp_elicitation: {
    label: '工具调用 MCP 交互',
    hint: '当 MCP 工具调用需要额外信息时通过交互方式向用户询问。配合 auth_elicitation 使用。',
  },
  tool_search: { label: '工具搜索', hint: '允许 Agent 搜索可用工具列表，在工具众多时快速找到合适的。' },
  tool_search_always_defer_mcp_tools: {
    label: '工具搜索总是延迟 MCP 工具',
    hint: '在工具搜索时总是延迟加载 MCP 工具，加快初始搜索速度。MCP 工具会在首次调用时加载。',
  },
  ...
  unavailable_dummy_tools: {
    label: '不可用占位工具',
    hint: '为不可用的工具显示占位条目，便于用户了解存在但未启用的工具。',
  },
  undo: { label: '撤销', hint: '允许撤销 Agent 的上一步操作（如文件修改、命令执行）。提供容错能力。' },
  unified_exec: {
    label: '统一执行',
    hint: '使用统一执行器替代多个独立执行工具，简化执行流程。与 experimental_use_unified_exec_tool 类似但更稳定。',
  },
  unified_exec_zsh_fork: {
    label: '统一执行 zsh fork',
    hint: '统一执行器使用 zsh fork 方式，提升隔离性。仅在 zsh 环境下生效。',
  },
  use_agent_identity: {
    label: '使用 Agent 身份',
    hint: '在 Git 提交等操作中使用 Agent 专属身份而非用户身份，便于区分人类和 Agent 的操作。',
  },
  use_legacy_landlock: {
    label: '使用旧版 Landlock',
    hint: '使用旧版 Linux Landlock 沙箱实现。仅在内核版本较旧时启用。',
  },
  use_linux_sandbox_bwrap: {
    label: '使用 Linux bwrap 沙箱',
    hint: '使用 bwrap 作为 Linux 沙箱实现，提供文件系统隔离。需要系统安装 bwrap。',
  },
  web_search: {
    label: '网页搜索',
    hint: '允许 Codex 在回答问题时主动检索互联网，获取最新资讯、文档或代码示例。关闭后只能依赖训练数据。',
  },
  web_search_cached: { label: '网页搜索(缓存)', hint: '使用缓存版的网页搜索，减少重复请求。结果可能略旧。' },
  web_search_request: {
    label: '网页搜索请求',
    hint: '暴露网页搜索请求工具，允许 Agent 直接发起搜索请求并获取原始结果。',
  },
  workspace_dependencies: { label: '工作区依赖', hint: '自动分析工作区的依赖关系，在执行命令前提示缺失的依赖。' },
  workspace_owner_usage_nudge: {
    label: '工作区所有者使用提示',
    hint: '向工作区所有者显示使用提示和统计，便于了解团队使用情况。',
  },
  apply_patch_preserve_line_endings: { label: 'apply_patch 更新文件时保留原有换行符' },
  background_paginated_rollout_migration: { label: '在后台把旧本地 rollout 文件迁移为分页历史' },
  bedrock_setup_wizard: { label: '在 TUI 登录引导中提供 Amazon Bedrock 配置向导' },
  code_mode_interrupt: { label: 'turn 被中断时终止正在执行的 code mode cell' },
  code_mode_prewarm: { label: '会话启动时即建立 code-mode host 连接' },
  compaction_image_budget: { label: '把保留的图片计入远程压缩上下文预算' },
  content_item_kinds: { label: '在内部 Responses 元数据中逐条发送内容分类' },
  cwd_relative_turn_diffs: { label: 'turn diff 展示路径使用当前工作目录' },
  guardian_enhanced_node_repl_transcripts: {
    label: '在 Guardian 复核中包含已完成的 node_repl / cua_repl code mode 响应',
  },
  guardian_ext: { label: '启用由扩展拥有的同步 Guardian 复核者' },
  guardian_node_repl_transcript_images: {
    label: '在 Guardian 复核中包含已完成的 node_repl / cua_repl code mode 响应图片',
  },
  guardian_reuse_parent_compaction: { label: '重启 Guardian 复核会话时复用已加密的父级压缩' },
  image_resize_notice: { label: '提示图片被缩放时告知模型并附上尺寸' },
  in_app_chat: { label: '允许桌面端 in-app 聊天面板（仅应由 requirements 设置）' },
  in_app_dictation: { label: '允许桌面端 in-app 听写（仅应由 requirements 设置）' },
  in_app_local_automation: { label: '允许桌面端运行本地自动化（仅应由 requirements 设置）' },
  mcp_oauth_refresh_coordination: { label: '允许 RMCP 通过配置的凭据存储协调 OAuth 刷新' },
  omit_app_server_notification_media: { label: '在 app-server 条目通知中省略内联图片与音频内容' },
  powershell_shell_version: { label: '暴露当前 PowerShell 执行宿主的主/次版本号' },
  psp: { label: '让第一方 ChatGPT 请求经由 PSP' },
  reasoning_effort_override: { label: '所选推理强度变化时追加受信任的响应配置条目' },
  retain_client_developer_messages: { label: '跨压缩的上下文窗口保留客户端编写的 developer 消息' },
  shell_snapshot_v2: { label: '把经策略过滤的 shell 快照完全保留在执行器内存中' },
  skip_host_skill_discovery: { label: '当没有已注册贡献者需要时跳过 host skill 快照' },
  step_model_switching: { label: '允许为后续 step 捕获显式请求切换模型' },
  transcript_v2: { label: '交互式转录 composer 与 turn 选择 UI（底层开发中）' },
  unbounded_connection_retries: { label: '网络连接失败时持续重试，保持采样 turn 存活' },
  unified_exec_tty: { label: '允许统一 exec 命令申请交互式终端' },
  unified_image_budget: { label: '对所有图片统一使用同一像素与 token 预算，忽略旧的 detail 提示' },
  view_image: { label: '内置本地图片查看工具' },
  windows_sandbox_service: { label: '尝试通过已安装的服务完成提权 Windows 沙箱准备' },
  worktrees: { label: '启用托管的 worktree 创建与仓库感知会话' },
  write_stdin_approval: { label: '向已提权的 unified-exec 终端写入输入前需要审批' },
};
...
const JSON_PARENT_KEYS = new Set(STATIC_SPECS.filter((spec) => spec.kind === 'json').map((spec) => spec.key));
...
function groupForUnknown(key: string): CodexSettingGroup {
  ...
  if (key.startsWith('mcp_') || key.startsWith('mcp_servers') || key.startsWith('model_providers'))
    return 'integrations';
...
function JsonValueEditor({
  ...
  value: CodexValue;
  onCommit: (value: CodexValue) => void;
  placeholder: string;
  t: Translate;
}) {
    ...
    <div style={{ width: '100%' }}>
      ...
      {invalid && (
        <div style={{ marginTop: 4, color: 'var(--status-error-default)', fontSize: 'var(--body-xs-font-size)' }}>
          {t('applications.codexSettings.invalidFieldJson')}
        </div>
      )}
    </div>
  );
...
function controlFor(
  spec: CodexSettingSpec,
  settings: Record<string, CodexValue>,
  onUpdate: (key: string, value: CodexValue) => void,
  t: Translate,
) {
    ...
    if (value !== null && typeof value === 'object') {
      return (
        <JsonValueEditor
          ...
          placeholder={t('applications.codexSettings.jsonPlaceholder')}
          t={t}
        />
      );
    }
  ...
  if (spec.kind === 'stringList') {
    return (
      <StringListEditor
        value={value}
        onCommit={(next) => onUpdate(spec.key, next)}
        placeholder={t('applications.codexSettings.listPlaceholder')}
      />
    );
  }
  if (spec.kind === 'json') {
...
// 与「模型来源」有关的字段：不接受在这里手改——未接管时它属于用户自己的
// Codex / 订阅配置（由 Codex 自身管理），接管后则完全由 Melody Hub 写入。
const MODEL_SOURCE_KEYS = new Set([
  'model',
  'model_provider',
...
export function CodexSettingsEditor({ settings, onSettingChange, t }: CodexSettingsEditorProps) {
  ...
  const filteredSpecs = useMemo(() => {
    const visible = specs.filter((spec) => !MODEL_SOURCE_KEYS.has(spec.key));
  ...
  const updateSetting = (key: string, value: CodexValue) => {
    ...
    <Card padding="0" style={{ overflow: 'hidden', marginTop: 'var(--spacer-16)' }}>
      ...
      >
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--spacer-6)' }}>
          <button
            ...
            onClick={() =>
              setOpenGroups(
                Object.fromEntries(GROUPS.map((group) => [group, true])) as Record<CodexSettingGroup, boolean>,
              )
            }
            title={t('applications.codexSettings.expandAll')}
          ...
          <button
            ...
            onClick={() =>
              setOpenGroups(
                Object.fromEntries(GROUPS.map((group) => [group, false])) as Record<CodexSettingGroup, boolean>,
              )
            }
            title={t('applications.codexSettings.collapseAll')}
            aria-label={t('applications.codexSettings.collapseAll')}
          >
            <Minimize2 size={14} />
          </button>
        </div>
      ...
      <div style={{ padding: 'var(--spacer-12) var(--spacer-20) 0' }}>
        <Input
          ...
          placeholder={t('applications.codexSettings.searchPlaceholder')}
          aria-label={t('applications.codexSettings.searchPlaceholder')}
        />
      </div>
      <div style={{ padding: 'var(--spacer-8) var(--spacer-20) var(--spacer-16)' }}>
        ...
        {!filteredSpecs.length && (
          ...
          >
            {t('applications.codexSettings.noResults')}
          </div>
        )}
      </div>
    </Card>
