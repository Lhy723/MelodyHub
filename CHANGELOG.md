...
- 设置页数据管理实现配置导出/导入（原为占位）：导出「设置 + 供应商 + 聚合」为单个版本化 JSON（API Key 按用户选择明文随文件带走，导入落盘前重新加密）；导入整体覆盖前展示文件内供应商/聚合数量并要求确认，导入后热更新代理、路由与开机自启
- 模型配置页清单解除 4 行定高上限（FilterGrid 新增 auto 模式），全屏可显示更多卡片

### 变更

- 首次启动（无设置文件）默认不再自动开启代理；已保存过设置的用户不受影响

## 0.1.15 (2026-09-09)## 0.1.15 (2026-09-09)

### 新增

- 供应商卡片新增实时速率行：按供应商显示最近 1 小时平均 TPM/RPM（滚动窗口，跟随健康状态轮询刷新）
- 近期调用记录从仪表盘独立为「调用记录」页面（侧边栏新入口）：保留时间范围切换、分页与详情抽屉；仪表盘新增按 Token 的第二个模型分布饼图（与调用次数饼图共用分组与图例）

### 修复

- Responses 入口接受 input item 的 `role: "system"`（OpenAI Responses API 合法角色；zcode 等客户端此前被 422 拒绝）

## 0.1.14 (2026-09-09)## 0.1.14 (2026-09-09)
