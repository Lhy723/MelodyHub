...
- 统一侧边栏页面切换动画：标题与页面内容使用更清晰的淡入、模糊渐清效果
- 模型配置页简化标题层级，副标题改为“管理客户端可调用的模型映射。”
- 应用设置状态改为更紧凑的横向标签，并补充命令路径展示
- 模型配置列表加宽上下文与输出字段，完整显示较大的数字

### 修复

- 保持 Agent 命令检测逻辑在不同平台上的 Clippy 检查通过

## 0.1.17 (2026-09-11)

### 修复

- Windows 启动时不再额外弹出独立小窗口，主窗口等待页面完成加载后显示，减少黑屏等待感
- 将后端启动初始化移到后台，避免代理、更新检查和本地配置初始化阻塞界面首帧
- 首次启动且未启用开机自启时，不再把不存在的启动项误报为错误
- 对齐 Tauri updater 的前端与 Rust 插件版本，避免包版本不一致提示

### 变更

- 应用设置页分别展示 Agent 命令和配置文件的检测状态，并显示已找到命令的路径
- 增加页面级加载状态，异步切换页面时不再出现空白内容

### 文档

- 更新 README 界面预览图；模型配置图使用不包含本地模型信息的脱敏示例
